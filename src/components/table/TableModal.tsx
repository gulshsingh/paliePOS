  ...
  const renderTable = ({ item }: { item: RestaurantTable }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => { if (!isOccupied) { onSelect(item); onClose(); } }}
        disabled={isOccupied}
        activeOpacity={0.8}>
        <View style={[styles.iconWrap, { backgroundColor: isOccupied ? theme.colors.dangerLight : theme.colors.successLight }]}>
        ...
        <Text style={[styles.tableName, isOccupied && styles.tableNameOccupied]} numberOfLines={1}>
          {item.name}
        </Text>
        <View style={styles.capacityRow}>
          <MaterialCommunityIcons name="account-outline" size={11} color={theme.colors.textMuted} />
          <Text style={styles.capacityText}>{item.capacity}</Text>
        </View>
        <View style={[styles.statusBadge, { backgroundColor: isOccupied ? theme.colors.dangerLight : theme.colors.successLight }]}>
          <Text style={[styles.statusText, { color: isOccupied ? theme.colors.danger : theme.colors.success }]}>
        ...
        <TouchableOpacity style={styles.sheet} activeOpacity={1}>
          ...
          {tables.length === 0 ? (
            <View style={styles.empty}>
              <MaterialCommunityIcons name="table-furniture" size={40} color={theme.colors.textMuted} />
...
const styles = StyleSheet.create({
  ...
  sheet:           { backgroundColor: '#fff', borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingHorizontal: 14, paddingBottom: 32, maxHeight: '75%' },
  ...
  card:            { width: '31%', backgroundColor: '#fff', borderRadius: theme.radius.lg, padding: 12, alignItems: 'center', gap: 6, borderWidth: 1.5, borderColor: theme.colors.successLight + 'AA', ...theme.shadow.sm },
  ...
  tableName:       { fontSize: 13, fontWeight: '800', color: theme.colors.textPrimary, textAlign: 'center' },
  tableNameOccupied: { color: theme.colors.textMuted },
  capacityRow:     { flexDirection: 'row', alignItems: 'center', gap: 3 },
  capacityText:    { fontSize: 11, color: theme.colors.textMuted, fontWeight: '600' },
