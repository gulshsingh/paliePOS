...
import { tableSchema, TableFormData } from '../../schemas/tables/tableSchema';
...
export default function TableFormScreen() {
  ...
  const [status,   setStatus]   = useState<'available' | 'occupied'>(table?.status ?? 'available');
  ...
  const validate = (): boolean => {
    const result = tableSchema.safeParse({ name, capacity: Number(capacity), status });
    if (!result.success) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEdit) { await updateTable.mutateAsync({ id: table.id, data }); }
      else        { await createTable.mutateAsync(data); }
      navigation.goBack();
  ...
  return (
    <KeyboardAvoidingView
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}
        ...
        <View style={styles.previewWrap}>
          ...
          <View style={[styles.previewStatus, { backgroundColor: currentStatus.bg }]}>
            ...
            <Text style={[styles.previewStatusText, { color: currentStatus.color }]}>{currentStatus.label}</Text>
          </View>
        </View>
        ...
        <View style={styles.card}>
          ...
          <TextInput
            ...
            value={name} onChangeText={(v) => { setName(v); setErrors((p) => ({ ...p, name: undefined })); }}
            placeholder="e.g. Table 1" placeholderTextColor={theme.colors.textMuted}
            onFocus={() => setFocused('name')} onBlur={() => setFocused(null)}
          />
          {errors.name ? <Text style={styles.fieldError}>{errors.name}</Text> : null}

          <Text style={[styles.label, styles.labelSpacing]}>Seating Capacity *</Text>
          <TextInput
            style={[styles.input, focused === 'cap' && styles.inputFocused, errors.capacity && styles.inputError]}
            value={capacity} onChangeText={(v) => { setCapacity(v); setErrors((p) => ({ ...p, capacity: undefined })); }}
            placeholder="e.g. 4" placeholderTextColor={theme.colors.textMuted}
            keyboardType="numeric" onFocus={() => setFocused('cap')} onBlur={() => setFocused(null)}
          />
          {errors.capacity ? <Text style={styles.fieldError}>{errors.capacity}</Text> : null}
        </View>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.statusRow}>
            {STATUS_OPTIONS.map((s) => (
              <TouchableOpacity key={s.key} activeOpacity={0.8}
                style={[styles.statusOption, status === s.key && { borderColor: s.color, backgroundColor: s.bg }]}
                onPress={() => setStatus(s.key as 'available' | 'occupied')}>
                <MaterialCommunityIcons name={s.icon} size={24} color={status === s.key ? s.color : theme.colors.textMuted} />
                <Text style={[styles.statusLabel, status === s.key && { color: s.color }]}>{s.label}</Text>
                {status === s.key && <View style={[styles.selectedDot, { backgroundColor: s.color }]} />}
              </TouchableOpacity>
            ))}
          </View>
        </View>
      ...
      <View style={styles.footer}>
        ...
        <TouchableOpacity style={[styles.saveBtn, isPending && styles.buttonDisabled]}
...
const styles = StyleSheet.create({
  ...
  tableIcon:       { width: 80, height: 80, borderRadius: 24, backgroundColor: '#fff', justifyContent: 'center', alignItems: 'center', borderWidth: 2, ...theme.shadow.sm },
  previewName:     { fontSize: 16, fontWeight: '800', color: theme.colors.textPrimary },
  previewStatus:   { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 12, paddingVertical: 4, borderRadius: theme.radius.full },
  ...
  input:           { backgroundColor: theme.colors.surfaceSecondary, color: theme.colors.textPrimary, borderRadius: theme.radius.md, paddingHorizontal: 14, paddingVertical: 12, fontSize: 14, borderWidth: 1.5, borderColor: theme.colors.border, marginBottom: 4 },
  ...
  fieldError:      { color: theme.colors.danger, fontSize: 11, fontWeight: '600', marginBottom: 6 },
  statusRow:       { flexDirection: 'row', gap: 12 },
  statusOption:    { flex: 1, alignItems: 'center', gap: 6, paddingVertical: 16, borderRadius: theme.radius.lg, borderWidth: 1.5, borderColor: theme.colors.border, backgroundColor: theme.colors.surfaceSecondary, position: 'relative' },
  statusLabel:     { fontSize: 13, fontWeight: '700', color: theme.colors.textMuted },
